...
describe('buildRows with pinned worktrees', () => {
  ...
  it('normalizes path-like same-folder project labels and exposes workspace modes', () => {
    ...
    // Why: upstream #11805 stabilizes cross-host project grouping by merging same-label
    // plain+worktree project surfaces into a single header with a 'mixed' workspace mode
    // (count:2) instead of two separate plain/worktree headers.
    expect(headers).toMatchObject([
      { type: 'header', label: 'Workf', workspaceMode: 'mixed', count: 2 }
    ])
  })
