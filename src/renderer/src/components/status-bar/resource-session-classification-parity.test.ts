...
describe('resource session classification parity', () => {
  it('feeds the row merge the same binding inputs as the bulk selector', () => {
    ...
    const mergeStart = source.indexOf('mergeSnapshotAndSessions(resourceSnapshot')
    const mergeTail = /worktreeById\r?\n\s+}\)/.exec(source.slice(mergeStart))
    expect(mergeStart).toBeGreaterThanOrEqual(0)
    expect(mergeTail).not.toBeNull()
    const mergeCall = source.slice(mergeStart, mergeStart + mergeTail!.index + mergeTail![0].length)
