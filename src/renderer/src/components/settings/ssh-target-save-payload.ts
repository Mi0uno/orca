...
export function buildSshTargetSavePayload(form: EditingTarget): SshTargetSavePayloadResult {
  ...
  // Why: password auth runs through ssh2; turn off config-derived GSSAPI so it does not select system SSH.
  ...
  return {
    ...
    payload: {
      ...
      updates: {
        ...
        savePassword,
        identityFile,
        gssapiAuthentication: gssapiAuthentication || undefined,
        proxyCommand,
        jumpHost,
