...
export type AgentStatusSlice = {
  ...
  /** User labels keyed by pane, separate so hook payloads cannot overwrite manual renames. */
  agentCustomTitlesByPaneKey: Record<string, string>

  /** Ephemeral launch snapshots keyed by pane; hook payloads lack Orca launch settings. */
  ...
  /** Close a pane while keeping resumable provider sessions as muted sidebar history. */
  retainClosedAgentSession: (paneKey: string) => void

  /** Convert resumable sessions under a closing terminal tab to sidebar history. */
  ...
  return {
    ...
    retainClosedAgentSessionsByTabPrefix: (tabIdPrefix) => {
      ...
      set((s) => {
        ...
        const sleepingKeys = Object.entries(s.sleepingAgentSessionsByPaneKey)
          .filter(([paneKey, record]) => paneKey.startsWith(prefix) || record.tabId === tabIdPrefix)
          .map(([paneKey]) => paneKey)
