...
export const createTerminalSlice: StateCreator<AppState, [], [], TerminalSlice> = (set, get) => ({
  ...
  closeTab: (tabId, opts) => {
    ...
    // resumable history before user/cleanup teardown, but leave natural PTY
    // exits' existing resume records untouched.
    if (retiresSession) {
      if (opts?.retainAgentHistory !== false) {
        get().retainClosedAgentSessionsByTabPrefix(tabId)
      }
      get().dropAgentStatusByTabPrefix(tabId, {
        worktreeId: retirementPlan.worktreeId ?? undefined
      })
    }
    set((s) => {
    ...
    // Why: retired pane keys never recur, so clear stranded foreground entries.
