...
export function normalizeRuntimePathForComparison(rawValue: string): string {
  return normalizeRuntimePathForComparisonWithFlavor(rawValue, 'auto')
}

function normalizeRuntimePathForComparisonWithFlavor(
  rawValue: string,
  pathFlavor: 'auto' | 'posix' | 'windows'
): string {
  // Normalize before any folding so the WSL alias branch below is covered too.
  const value = rawValue.normalize('NFC')
  const isWindowsPath =
    pathFlavor === 'windows' || (pathFlavor === 'auto' && isWindowsAbsolutePathLike(value))
...
export function relativePathInsideRoot(rootPath: string, candidatePath: string): string | null {
  ...
  const rootNfc = rootPath.normalize('NFC')
  const candidateNfc = candidatePath.normalize('NFC')
  const pathFlavor =
    isWindowsAbsolutePathLike(rootNfc) || isWindowsAbsolutePathLike(candidateNfc)
      ? 'windows'
      : 'posix'
  ...
  const comparisonRoot = normalizeRuntimePathForComparisonWithFlavor(rootPath, pathFlavor)
  const comparisonCandidate = normalizeRuntimePathForComparisonWithFlavor(candidatePath, pathFlavor)
