...
describe('cross-platform path containment', () => {
  ...
  it('matches slash-normalized Windows roots against backslash directory entries', () => {
    const sessionsRoot = resolveRuntimePath('\\Users\\Example\\.codex', 'sessions')

    expect(
      relativePathInsideRoot(sessionsRoot, '\\Users\\example\\.codex\\sessions\\2026\\07')
    ).toBe('2026/07')
  })
})
