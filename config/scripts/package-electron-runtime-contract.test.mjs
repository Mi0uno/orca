...
describe('Electron runtime package contract', () => {
  ...
  it('preflights SignPath module install before Windows signing side effects', () => {
    ...
    // Why: the fork gates signing behind a secret-presence resolve step and delegates the
    // module install to the shared composite action, so the workflow step must `uses` it
    // (no inline `run`) and run only when the resolve step enabled signing.
  ...
  it('keeps terminal rendering regressions in the manual golden E2E workflow', () => {
    ...
    // Why: the fork makes golden E2E non-blocking for non-official repos so a fork release cut still
    // publishes the Windows installer when the copied mac/Linux golden jobs are unhealthy or unavailable.
